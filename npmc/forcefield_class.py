    ...
    def tabulate_pdf_weighted(self,intervals,write):
        ...
        for i in range(intervals):
            for j in range(intervals): 
                ...
                weights[i*intervals+j] = si.dblquad(self.unnorm_prob,pdf[i*intervals+j,0],pdf[i*intervals+j,1],pdf[i*intervals+j,2],pdf[i*intervals+j,3])[0]
    ...
    def tabulate_pdf(self,intervals1,intervals2,write):
        ...
        for i in range(intervals1):
            ...
            for j in range(intervals2-1):
                ...
                boundary=so.brenth(lambda phi:si.dblquad(self.unnorm_prob,pdf[i,j,0],pdf[i,j,1],pdf[i,j,2],phi)[0]/self.Q-(1/intervals1)*(1/intervals2),pdf[i,j,2],upper_bracket)
                pdf[i,j,3] = boundary; pdf[i,j+1,2] = boundary
                probs[i,j] = si.dblquad(self.unnorm_prob,pdf[i,j,0],pdf[i,j,1],pdf[i,j,2],pdf[i,j,3])[0]/self.Q                
            probs[i,-1] = si.dblquad(self.unnorm_prob,pdf[i,-1,0],pdf[i,-1,1],pdf[i,-1,2],pdf[i,-1,3])[0]/self.Q
