...
def initializeMols(atoms,bonds):
    ...
    sulfurMols = atoms[np.where(atoms[:,2]==sulfurID)][:,1]
    ddtMols = np.intersect1d(atoms[np.where(atoms[:,2]==ch3ID)][:,1],sulfurMols)
    meohMols = np.intersect1d(atoms[np.where(atoms[:,2]==oxygenID)][:,1],sulfurMols)
    ethMols = np.intersect1d(atoms[np.where(atoms[:,2]==oxygenID)][:,1],atoms[np.where(atoms[:,2]==ch3ID)][:,1])
    ...
    eth_ch3s = [atom[0] for atom in atoms if (atom[2]==ch3ID and (atom[1] in ethMols))]

    ddts = np.empty([ddtMols.shape[0],13])
    meohs = np.empty([meohMols.shape[0],5])
    eths = np.empty([ethMols.shape[0],4])          

    for idx,(ddtsulfur,meohsulfur,eth_ch3) in enumerate(itt.izip_longest(ddtsulfurs,meohsulfurs,eth_ch3s)):
        ...
        if(not (eth_ch3==None)):
            eths[idx,:] = getMoleculeAtoms(bonds,eth_ch3)   
    return (ddts,meohs,eths)
